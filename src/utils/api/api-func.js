...
import { G_CONFIG, MINIID, ISDEMO } from '../config';
...
const ApiFunc = {
    ...
    http: async (url = '', option = {}) => {
        const opionSend = {
            ...
            header: {
                ...getCommonHeader(),
                adtag:
                    (wepy.$instance && wepy.$instance.globalData.adtag) || '',
        ...
        try {
            const { res } = await CommonFunc.wepyFunc(
                'request',
                opionSend,
    ...
    post: async (url = '', data = {}) =>
        await ApiFunc.http(url, { method: 'post', data }),

    get: async (url = '', data = {}) =>
        await ApiFunc.http(url + CommonFunc.serialize(data), { method: 'get' }),
