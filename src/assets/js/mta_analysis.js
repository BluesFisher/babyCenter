...
function getExtentInfo() {
    ...
    return {
        ...
        ext:
            'v=' +
            MTA_CONFIG.version +
            (a !== null && a !== '' ? ';ui=' + encodeURIComponent(a) : '')
    };
}
function getQuery(a) {
...
function ignoreParams(a) {
    if (MTA_CONFIG.ignore_params.length < 1) return a;
    var b = {},
        c;
    for (c in a) MTA_CONFIG.ignore_params.indexOf(c) >= 0 || (b[c] = a[c]);
    return b;
}
function initOnload() {
    var a = Page;
